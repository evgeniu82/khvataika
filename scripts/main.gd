...
const FIRST_5_TEST: bool = true
# Test build: only feature groups 1-5 are enabled; groups 6-22 remain at the stable baseline.
...
func get_server_game_state() -> Dictionary:
    return {
        "save_schema": SAVE_SCHEMA_VERSION,
            ...
            "coins": coins, "rating_points": rating_points, "player_name": player_name, "player_avatar_index": player_avatar_index,
...
func apply_server_game_state(data: Dictionary) -> void:
    ...
    rating_points = maxi(0, int(data.get("rating_points", rating_points)))
...
func add_rating_for_win(rarity: String) -> void:
    rating_points += rating_for_rarity(rarity)
...
func finalize_delivered_prize() -> void:
    ...
    if kind == "capsule":
        ...
        total_prizes_won += 1
    add_rating_for_win(last_prize_rarity)
...
func check_achievements() -> void:
    for spec in achievement_specs:
        ...
        if achievement_value(spec) >= int(spec["value"]):
            ...
            var reward := 25 + mini(125, int(spec.get("value", 1)) * 2)
            coins += reward
            pending_new_achievements.append(String(spec["name"]))
            current_result = "🏆 ДОСТИЖЕНИЕ: %s • +%d ₽" % [String(spec["name"]), reward]
    if not pending_new_achievements.is_empty():
